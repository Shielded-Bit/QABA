...
export default function SettingsLayout({ children }) {
  ...
  const isProfileActive = pathname === '/agent-dashboard/settings' || pathname === '/agent-dashboard/settings/profile';
  const isPasswordActive = pathname === '/agent-dashboard/settings/password';
  ...
  <h1 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-800  to-[#3ab7b1]">Settings</h1>
              ...
              <nav className="hidden md:flex space-x-6">
                ...
                <button
                  onClick={() => navigateTo('/agent-dashboard/settings/password')}
                  className={`inline-flex items-center px-4 py-2 rounded-md text-sm font-medium transition-colors duration-200 ${
                    isPasswordActive 
                      ? 'bg-blue-50 text-blue-700 border-b-2 border-blue-600' 
                      : 'text-gray-600 hover:text-blue-600 hover:bg-gray-100'
