    ...
    try {
      ...
      if (!response.success) {
        ...
        if (response.errors?.admin_user) {
          // Handle admin user error more gracefully
          if (isDraft) {
            // For drafts, we'll allow saving even without an admin user
            toast.success('Property saved as draft locally');
            return;
          }
          
          toast.error('Property submission requires admin review. Please contact support.');
          return;
        }
